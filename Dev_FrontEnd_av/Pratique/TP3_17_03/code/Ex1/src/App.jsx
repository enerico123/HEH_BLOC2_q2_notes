...
const searchAuthor = (event) => {
  const termSearched = event.target.value.toLowerCase();
  const filteredPhoto = photos.filter((photo) => {
    return photo.author.toLowerCase().includes(termSearched);
  });
  setFilteredPhotos(filteredPhoto);
}
...
const hideId = (event) => {
  ...
  return (
    <>
      <div><Search searchAuthor={searchAuthor} hideId={hideId}/></div>
      
      <Gallery photos={filteredPhotos} hideId={isChecked}/>
    </>
  )
}
